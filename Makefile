...
create:
	$(DOCKER_COMPOSE) run game_app yarn create vite . --template vue-ts
	$(DOCKER_COMPOSE) run game_app npx eslint --init
	$(DOCKER_COMPOSE) run game_app yarn add -D vitest
	$(DOCKER_COMPOSE) run game_app yarn add -D husky
